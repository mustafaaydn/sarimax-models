...
from sgd_sarimax import SARIMAX_Base
...
class SARIMAX_KF(SARIMAX_Base):
    ...
    def _set_params(self, *args):
        ...
        # if p > 0:
        #     estimates[:p] = constrain_stationary_univariate(estimates[:p])
        # if P > 0:
        #     estimates[p:p + P] = 1 / estimates[p:p + P]
...
if __name__ == "__main__":
    ...
    m3data = pd.read_excel(r"c:\users\need-\downloads\m3c.xls",
                           sheet_name="M3Month")
    kpss = sm.tsa.kpss
    adf = sm.tsa.adfuller

    def getdata(n=None):
        md = m3data.sample() if n is None else pd.DataFrame(m3data.iloc[n]).T
        print(*md.loc[:, "Series"].items())
        md.columns = md.columns.map(str)
        return md.loc[:, "1":].dropna(axis=1).to_numpy().squeeze().astype("f")

    from IPython import get_ipython
    try:
        get_ipython().run_line_magic("matplotlib", "")
    except AttributeError:
        print("running on non-i-python.")
        import matplotlib.pyplot as plt

    from utils import plot_cf, plot_ts
    plot_ts, plot_cf
    ...
    for _ in range(5):
        ...
        print("\tFound orders! \n" f"\t{(p, d, q)}" "\t" f"{(P, D, Q, m)}")
        if p == q == P == Q == 0:
            print("\t and so data seemed noise..\n")
            continue
        ...
        sta = sm.tsa.SARIMAX(endog, order=tkf.order,
                             seasonal_order=tkf.seas_order)
        stafit = sta.fit(disp=0)
        preds_sta = stafit.predict()
        fors_sta = stafit.forecast(h)

        # Pure ARIMA of statsmodels (N/A for seasonals, again MLE with KF)
        ta = sm.tsa.arima.ARIMA(endog, order=tkf.order)
        tafit = ta.fit()
        preds_ta = tafit.predict()
        fors_ta = tafit.forecast(h)

        fig = plot_ts(mda[m * D + d:],
                      np.r_[preds_sta[m * D + d:], fors_sta],
                      np.r_[preds_kf, fors_kf],
                      np.r_[preds_sgd, fors_sgd],
                      legend=["endog",
                              "Statsmodels",
                              "Our Kalman Filter Way",
                              "Our SGD Way"],
                      title="Forecasts & In Sample Predictions",
                      )
        test_boundary = mda.size - (m * D + d) - 1 - h
        try:
            plt.show()
            plt.axvline(test_boundary, c="k")
        except NameError:
            import matplotlib.pyplot as plt
            print("\n i-python-i \n")
        fig.axes[0].axvline(x=test_boundary, c="k")
